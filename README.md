...
```bash
docker pull jasl8r/mattermost:3.4.0-1
```
...
```bash
docker run --name mattermost -d [OPTIONS] jasl8r/mattermost:3.4.0-1
```
...
docker run -it --name mattermost -d --link mattermost-postgres:postgres \
     --volume /srv/docker/mattermost/mattermost:/opt/mattermost/data \
     jasl8r/mattermost:3.4.0-1 app:migrate
```
